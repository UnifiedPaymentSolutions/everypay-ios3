...
@interface ACFloatingTextField ()
{
    ...
    NSLayoutConstraint *errorLabelHeight;
...
- (void)setPlaceholderText:(NSString *)placeholderText {
    ...
    [self setPlaceholderLabel:placeholderText];
    [self floatTheLabel];
}
...
- (void)setSelectedPlaceHolderTextfieldColor:(UIColor *)selectedPlaceHolderTextfieldColor {
    _selectedPlaceHolderTextfieldColor = selectedPlaceHolderTextfieldColor;
    [self floatTheLabel];
}
...
-(void)addErrorPlaceholderLabel{
    ...
    NSLayoutConstraint *topConstraint = [NSLayoutConstraint constraintWithItem:self.labelErrorPlaceholder attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:bottomLineView attribute:NSLayoutAttributeBottom multiplier:1.0 constant:kPlaceholderHeight];
    NSLayoutConstraint *trailingConstraint = [NSLayoutConstraint constraintWithItem:self.labelErrorPlaceholder attribute:NSLayoutAttributeTrailing relatedBy:NSLayoutRelationEqual toItem:bottomLineView attribute:NSLayoutAttributeTrailing multiplier:1.0 constant:5];
    NSLayoutConstraint *leadingConstraint = [NSLayoutConstraint constraintWithItem:self.labelErrorPlaceholder attribute:NSLayoutAttributeLeading relatedBy:NSLayoutRelationEqual toItem:bottomLineView attribute:NSLayoutAttributeLeading multiplier:1.0 constant:5];
    errorLabelHeight = [NSLayoutConstraint constraintWithItem:self.labelErrorPlaceholder
                                                          attribute:NSLayoutAttributeHeight
                                                          relatedBy:NSLayoutRelationEqual
                                                             toItem:nil
                                                          attribute: NSLayoutAttributeNotAnAttribute
    ...
    [self.labelErrorPlaceholder addConstraint:errorLabelHeight];
...
-(void)showErrorPlaceHolder{
    ...
    if (self.errorText != nil && ![self.errorText isEqualToString:@""]) {
        errorLabelHeight.constant = kPlaceholderHeight;
...
-(void)floatPlaceHolder:(BOOL)selected {
    ...
    if (selected) {
        ...
        bottomLineViewHeight.constant = 2;
        self.attributedPlaceholder = [[NSAttributedString alloc] initWithString:_placeholderText ? _placeholderText : self.placeholder attributes:@{NSForegroundColorAttributeName:self.selectedPlaceHolderTextfieldColor}];
